...
func generate_paths(start:Vector2, end:Vector2, num_paths:int):
	...
	for i in range(num_paths):
		...
		while current_pos.distance_to(end) > 2.0: #stop when close enough
			...
			var next_step = current_pos + (direction + random_offset) * 3.0
